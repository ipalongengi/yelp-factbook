...
if __name__ == '__main__':
    ...
    def find_stuff(city, biz_rdd, review_rdd):
        ...
        biz_rat_total.saveAsTextFile('%s_biz_review_total' % city)
        ...
        city_biz_top = biz_rdd.join(biz_rat_top) \
                              .sortBy(lambda x: x[1][1], ascendind=False) \
                              .map(lambda x: x[1])
        ...
        city_biz_bottom = biz_rdd.join(biz_rat_bottom) \
                                .sortBy(lambda x: x[1][1], ascendind=False) \
                                .map(lambda x: x[1])
